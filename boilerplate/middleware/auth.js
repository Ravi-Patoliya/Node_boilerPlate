...
module.exports = {
  auth: ({ isTokenRequired = true, usersAllowed = [] } = {}) => {
    return async (req, res, next) => {
      ...
      if (!decoded?._id && !decoded.email) { return response.UNAUTHORIZED({ res, message: MESSAGE.INVALID_TOKEN, }); }
      ...
      if (!user) return response.UNAUTHORIZED({ res, message: MESSAGE.INVALID_TOKEN });

      //* check if user is active or not
      if (!user.isActive) return response.UNAUTHORIZED({ res, message: MESSAGE.ACCOUNT_DISABLED });
