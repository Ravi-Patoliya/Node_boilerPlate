...
module.exports = async (error, req, res, next) => {
  if (error instanceof JsonWebTokenError || error instanceof NotBeforeError || error instanceof TokenExpiredError)
    response.UNAUTHORIZED({ res, message: MESSAGE.UNAUTHORIZED, payload: { context: error.message }, });

  else if (error instanceof MulterError)
    response.BAD_REQUEST({ res, message: MESSAGE.FILE_UPLOAD_FAILED, payload: { context: error.code }, });
    ...
    response.BAD_REQUEST({ res, message: MESSAGE.FAILED, payload: { context: error.message }, });
  }
    ...
    response.BAD_REQUEST({ res, message: MESSAGE.FAILED, payload: { context: error.message, error }, });

  else
    response.INTERNAL_SERVER_ERROR({ res, message: MESSAGE.INTERNAL_SERVER_ERROR, payload: { context: error.message, error }, });
